...
export class CommentApiService {
  ...
  getComments(page = 0, size = 10): Observable<any> {
    const params = new HttpParams().set('page', page).set('size', size);
    return this.httpClient.get<Comment[]>(`${this.baseUrl}`, { params });
  }

  getCommentsByThread(threadId: number, page = 0, size = 10): Observable<any> {
